...
def call_tool(tool_name, params):
    ...
    pb = (
        load_pybaseball()
    )  # Assuming load_pybaseball() is robust or its errors can propagate
    ...
    try:
        ...
        error_message = (
            f"Error executing tool '{tool_name}' with params '{params}': {str(e)}"
        )
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "type": {
                    "type": "string",
                    "description": "Type of stats (batting or pitching), defaults to 'batting' if not provided.",
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "serverInfo": {"name": "MLB Stats MCP", "version": "1.0.0"},
                },
                "id": rpc_id,
            }
            ...
            return {
                ...
                "result": {},
                "id": rpc_id,
            }  # Simple pong with empty object
        elif method == "tools/call":
            print(
                f"[TOOLS_CALL_HANDLER] Received JRPC params: {params}"
            )  # Log incoming params
            ...
            if actual_tool_params is None:
                actual_tool_params = params.get(
                    "arguments"
                )  # Common alternative, as seen from client
            ...
            print(
                f"[TOOLS_CALL_HANDLER] Derived tool_name: '{actual_tool_name}', derived_params: {actual_tool_params}"
            )  # Log derived params

            if actual_tool_name in [
                "get_player_stats",
                "get_team_stats",
                "get_leaderboard",
            ]:
                ...
                return {
                    ...
                    "error": {
                        "code": -32601,
                        "message": f"Tool '{raw_tool_name}' (parsed as '{actual_tool_name}') not found or supported.",
                    },
                    "id": rpc_id,
                }
...
async def mcp_tools_list():
    ...
    return {"jsonrpc": "2.0", "result": {"tools": STATIC_TOOLS}, "id": 1}
...
async def mcp_initialize(request: Request):
    ...
    try:
        ...
        return {"jsonrpc": "2.0", "error": {"code": -32000, "message": str(e)}, "id": 1}
...
def get_player_stats(
    name: str, start_date: Optional[str] = None, end_date: Optional[str] = None
):
    """
    ...
    try:
        ...
        player_df = pb.retrosheet.player_lookup(
            first_name=first, last_name=last, return_pandas=True
        )
        if player_df.empty:
        ...
        if start_date and end_date:
            stats = pb.statcast.statcast_date_range_pitch_by_pitch(
                start_date, end_date, return_pandas=True
            )
        else:
            # Default to last 30 days if no date range provided
            today = datetime.today().strftime("%Y-%m-%d")
            thirty_days_ago = (datetime.today() - datetime.timedelta(days=30)).strftime(
                "%Y-%m-%d"
            )
            stats = pb.statcast.statcast_date_range_pitch_by_pitch(
                thirty_days_ago, today, return_pandas=True
            )
...
def get_team_stats(team: str, year: int, type: str = "batting"):
    ...
    if year > current_server_year + 1:  # Allow current year and next year only
        error_msg = (
            f"Invalid year: {year}. Year cannot be more than 1 year in the future."
        )
        print(error_msg)
...
def get_leaderboard(stat: str, season: int, type: str = "batting"):
    ...
    if season > current_year + 1:  # Allow current year and next year only
        error_msg = (
            f"Invalid season: {season}. Year cannot be more than 1 year in the future."
        )
        print(error_msg)
        raise ValueError(error_msg)
